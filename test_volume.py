...
class TestVolume(unittest.TestCase):
    ...
    def test_load_archive_volume(self):
        self.test_volume3d_instance.mm_resolution = 0.6
        volume_saver = VolumeSaver(self.test_volume3d_instance)

        with NamedTemporaryFile(suffix=".tar.gz") as temporary_file:
            for i, total in volume_saver.save_volume_as_archive_iterably(
                archive_path=temporary_file.name, extension="png"
            ):
                pass

            volume_loader = VolumeLoader(temporary_file.name)
    ...
    def test_volume_load(self):
        ...
        with TemporaryDirectory() as temporary_directory:
            for i, total in volume_saver.save_files_iterably(
                destination_directory=temporary_directory, extension="png"
            ):
                pass

            with self.assertRaises(AssertionError):
                volume_loader = VolumeLoader(
                    volume_path=temporary_directory + "_"
                )
